...
- **Editing the footer or school marquee?** Don't touch the per-page HTML — those blocks are partial-injected at build time. Edit `partials/footer-en.html`, `partials/footer-zh.html`, `partials/school-marquee-{en,zh}.html`. Then run `python3 scripts/inject-partials.py` locally (or just commit — the Cloudflare build runs it). The `<!-- include: NAME -->` / `<!-- endinclude: NAME -->` markers in `public/**/*.html` are the rewrite targets. The two `404.html` files are the exceptions — they hand-roll their footers and have no markers, so direct edits there DO survive builds.
- **Adding analytics, or "wiring up" Matomo?** ⚠️⚠️ **It is already wired — do not add a snippet.** `public/js/matomo.js` has tracked to `analytics.forhuman.ca` with Site ID 1 since 2026-05-29, and it is loaded by every hand-written page plus `blog/layouts/_default/baseof.html`, so all 37 built pages including both `/writing/` trees are covered. A second tracker double-counts every page view, because both push to the same `window._paq`; that was done and reverted on 2026-09-07 (`8d5f266` → `f93b0c7`) and only a browser test caught it. ⚠️ **`grep -rn "_paq" --include='*.html'` returns nothing and is not evidence** — the tracking lives in a `.js` file. Ask `command grep -rl "js/matomo.js" public --include='*.html'`, or load a page and read what it POSTs. Server side: `~/code/infra/infra-nas/runbooks/matomo-finish-setup.md`.
...
- **Tidying `public/brand/`?** Several files there are referenced by **nothing in this site** and are not dead. `human-comma-email.png` is hot-linked by Nico's email signature (`docs/email-signature.html`); deleting it or moving its URL breaks every message he has already sent, which is exactly the rot this hosting was chosen to avoid. `human-wordmark-email.png` and `profile.png` are built for the same purpose but are not in the shipped signature — keep them, they are the alternates if it is ever redesigned. Treat `public/brand/` as a public asset contract, not as site assets.
