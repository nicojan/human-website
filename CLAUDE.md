...
- **Tidying `public/brand/`?** Several files there are referenced by **nothing in this site** and are not dead: `human-comma-email.png`, `human-wordmark-email.png` and `profile.png` are hot-linked from outside, by Nico's email signature (`docs/email-signature.html`). Deleting them breaks every message he has already sent, which is exactly the rot this hosting was chosen to avoid. Treat `public/brand/` as a public asset contract, not as site assets. Same reason the URLs must never move.
